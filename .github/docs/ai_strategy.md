...
## Overview
The `ai_strategy` block provides weight-based directives for AI behavior, monitoring game state and reacting instantly to triggers. It controls production ratios, naval dominance, coast defense, and avoidance strategies.

## Location
- Files: `common/ai_strategy/vnr_naval_strategy.txt`, `common/ai_strategy/naval_production.txt`

## Structure
- Strategies: Named blocks with `allowed`, `enable` (triggers), `ai_strategy` entries (type, id, value).
- Types: `naval_dominance`, `coast_defense`, `naval_avoid_region`, `role_ratio` (for production).

## Examples
- `ENG_protect_home_waters`: Enables on war with GER and navy size >30, sets `naval_dominance` in `vnr_eng_home_waters` (value 100).
- Production: `role_ratio` for `naval_capital_bb` (value 10).

## Usage
- Drives production via equipment roles from `ai_equipment`.
- Targets areas from `ai_area` or regions.
- Trigger: `enable_vnr_naval_ai` (scripted in `common/scripted_triggers/_vnr_scripted_triggers.txt`, game-rule enabled).

## Key Points
- Instant reactions to game state changes.
- Complements `ai_navy` goals (strategy sets priorities, goals execute).
- Weights influence AI focus (e.g., higher value = stronger emphasis).
