...
## Overview
The `ai_equipment` block specifies equipment variants that the AI will prioritize for production and design. It includes priorities for variant creation and roles that tie into production ratios set in `ai_strategy`.

## Location
- Files: `common/ai_equipment/*.txt` (e.g., `vnr_battleships.txt`, `vnr_carriers.txt`)

## Structure
Each equipment category has:
- `category`: e.g., `naval`
- `roles`: List of roles (e.g., `vnr_naval_capital_bb`)
- `priority`: Base value with modifiers (e.g., tech availability, expert AI)
- Variants: Named entries with `name`, `role_icon_index`, `priority`, and `target_variant` (specifying hull type and modules)

## Examples
- `naval_capital_battleship`: Role `vnr_naval_capital_bb`, variants like `capital_battleship_1924` with hull `vnr_ship_hull_heavy_2` and module upgrades.

## Usage in Production
- Roles are referenced in `ai_strategy/naval_production.txt` for `role_ratio` strategies.
- Priorities determine AI's eagerness to create new variants based on definitions and tech.

## Key Points
- Variants are event-triggered on game start, echoing `version_name` in OOB files.
- Modules are defined in `common/units/equipment/modules/ship_modules_*.txt`.
